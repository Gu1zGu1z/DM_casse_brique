...
def niveau_generation(niveau=1):
    ...
    if (niveau == 1):
        ...
        tab.append({ 'x' : 60, 'y' : 80, 'w' : 30, 'h' : 3, 'type' : 'facile', 'vie' : 1, 'couleur' : 3})
...
def draw():
    ...
    if (config['vies'] < 1):
        ...
        pyxel.blt(plateau_x, plateau_y, img(0), 128, 0, config['plateau_w'], config['plateau_h'])
        ...
        for b in tableau:
            if (b['vie'] > 0):
                pyxel.rectb(b['x'], b['y'], b['w'], b['h'], b['couleur'])
